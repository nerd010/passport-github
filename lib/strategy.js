...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'http://192.168.1.196:8080/authorize';
  options.tokenURL = options.tokenURL || 'http://192.168.1.196:8080/access_token';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.customHeaders = options.customHeaders || {};

  if (!options.customHeaders['User-Agent']) {
    options.customHeaders['User-Agent'] = options.userAgent || 'passport-bqj';
  }

  OAuth2Strategy.call(this, options, verify);
  this.name = 'bqj';
  this._userProfileURL = options.userProfileURL || 'http://192.168.1.196:8080/userInfo';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    profile.provider  = 'bqj';
    profile._raw = body;
    profile._json = json;


    if (self._scope && self._scope.indexOf('user:email') !== -1) {
      self._oauth2._request('GET', self._userProfileURL + '/emails', { 'Accept': 'application/vnd.bqj.v3+json' }, '', accessToken, function(err, body, res) {
        if (err) {
